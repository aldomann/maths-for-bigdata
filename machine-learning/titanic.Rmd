...
Below we can see plot of the error rates of the model:
```{r}
...
Since this is an academic problem, the solution is known and can be used to test the accuracy of our predictive model. First we read the data:
```{r}
# Source: https://raw.githubusercontent.com/shezard/Titanic/master/data/solution.csv
titanic.solution <- data.table::fread("data/solution.csv")
```
