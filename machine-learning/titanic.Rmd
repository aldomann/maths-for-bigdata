...
titanic.full %>% 
	...
	na.omit() %>% 
	ggplot() +
		aes(x = as.factor(deck), y = fare) +
		geom_boxplot() +
		labs(x = "Deck", y = "Fare (USD)")
```
...
vars.importance <- data.frame(variables = row.names(importance),
...
Now we can use the model to predict the real test data:
```{r}
...
```{r eval=FALSE}
write_csv(titanic.test.pred, "kaggle_titanic_predictions.csv")
```

We made two submissions:

- Not including `deck` with an accuracy of 0.77511
- Including `deck` with an accuracy of 0.75598

Apparently, the method to generate missing `deck` values is not that accurate. This could mean that `NA` values in deck are actually significative, or that the `deck` by itself just adds noise.
