...
We are going to use the `tidyverse` meta-package mainly for `dplyr` (data manipulation) and `magrittr` (to use pipes). For building the model, we are going to use the `randomForest` package.
```{r message=FALSE}
library(tidyverse)
