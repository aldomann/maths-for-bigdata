...
```{r}
titanic.train <- titanic.full[201:891,]
titanic.true.test <- titanic.full[1:200,]
titanic.test <- titanic.full[892:1309,]
```


```{r}
titanic.train.small <- titanic.train %>% select(- c("name" ,"ticket", "cabin", "surname"))

rf.titanic <- randomForest(survived~., data = titanic.train.small, na.action = na.omit, importance = TRUE)

plot(rf.titanic)

ggplot(cbind(rf.titanic$err.rate, 
						 tree = )) +
	aes(x = )
	geom_line(aes())
```
