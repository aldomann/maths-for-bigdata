---
title: 'Delivery 3: Isa Stuff'
author: "Alfredo Hernández"
date: "15 June 2018"
output:
  html_document:
    theme: cosmo
    highlight: tango
    toc: true
    toc_float:
      collapsed: false
      smooth_scroll: false
subtitle: Mathematics for Big Data
---

# Introduction

The sinking of the RMS Titanic is one of the most infamous shipwrecks in history.  On April 15, 1912, during her maiden voyage, the Titanic sank after colliding with an iceberg, killing 1502 out of 2224 passengers and crew. This sensational tragedy shocked the international community and led to better safety regulations for ships.

One of the reasons that the shipwreck led to such loss of life was that there were not enough lifeboats for the passengers and crew. Although there was some element of luck involved in surviving the sinking, some groups of people were more likely to survive than others, such as women, children, and the upper-class.

## Objective

The objective is to build a model with Machine Learning that can predict the survival chance of a given individual performing a binary classification.

## Data


# Cleaning
```{r message=FALSE}
library(tidyverse)
```

First of all, we need to read the 
```{r}
train <- as_tibble(data.table::fread("data/titanic_train.csv"))
test <- as_tibble(data.table::fread("data/titanic_test.csv"))
```


## Dealing with NAs

First of all, we check how many the training dataset has:
```{r}
train %>%
	summarise_all(funs(sum(is.na(.))))
```

### Cleaning empty fields

But that's only half of the picture, as there are some predictors that have no content (they are empty), and need to be changed into `NA`s.
```{r}
train <- train %>%
	mutate_all(funs(replace(., . == "", NA)))
```

Now we can see that we are taking into consideration all actual `NA`s:
```{r}
train %>%
	summarise_all(funs(sum(is.na(.))))
```

### Cleaning NAs


## Factors
```{r}
train <- train %>% 
	mutate_at(c("Survived", "Sex", "Ticket", "Cabin", "Embarked"), funs(factor(.)))
```


## Feature engineering

## Extracting information from the names
```{r}
train <- train %>% 
	mutate(Name = gsub("^((\\w+\\W+){4}\\w+).*$","\\1", Name)) %>% 
	mutate(Title = str_extract(Name, "[^,]+[.*]")) %>% 
	mutate(surname = str_extract(Name, "[^,]+[^,]")) %>% 
	mutate_at(c("Title", "surname"), funs(factor(.))) %>% 
	select(- Name)
```

### Ethnicities
```{r}
library(wru)

train.test <- as_tibble(predict_race(train, surname.only = T))

train.test <- train.test %>% 
	mutate(Ethnicity = colnames(
		train.test %>% dplyr::select(starts_with("pred."))
		)[max.col(
			train.test %>% dplyr::select(starts_with("pred.")),
			ties.method="first"
			)]) %>% 
	dplyr::select(- starts_with("pred.")) %>% 
	dplyr::rename(Surname = surname) %>% 
	mutate(Ethnicity = as.factor(Ethnicity))

train.test$Ethnicity <- train.test$Ethnicity %>%
	plyr::revalue(c("pred.whi" = "White",
									"pred.bla" = "Black",
									"pred.asi" = "Asian",
									"pred.his" = "Hispanic",
									"pred.oth" = "Other"))
```

```{r}
train.test %>% 
	dplyr::group_by(Sex, Ethnicity) %>% 
	select(Age) %>% 
	summarise(mean(Age, na.rm = T))
```




# Random Forest
```{r, message = FALSE}
library(randomForest)
```

```{r}
train.small <- train.test %>% drop_na() %>% select(- c("Ticket", "Cabin", "Surname"))

rf.titanic <- randomForest(Survived~., data = train.small, na.action = na.omit, importance = TRUE)

plot(rf.titanic)
```


